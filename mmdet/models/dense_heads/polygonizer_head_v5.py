...
class PolygonizerHeadV5(MaskFormerHead):
    ...
    def __init__(self,
                 ...
                 interpolate_type: str = 'fixed_length',
                 init_poly_type: str = 'predicted',
        ...
        self.init_poly_type = init_poly_type
        ...
        self.poly_reg_embed = nn.Sequential(
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, self.num_inter_points * 2))
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json):
        ...
        inter_gt_torch = polygon_utils.interpolate_ring(poly_gt_torch, num_bins=self.num_inter_points, pad_length=self.num_inter_points)

        if self.poly_align_type == 'sort_by_angle':
            reg_targets = polygon_utils.sort_rings_by_angle(inter_gt_torch)

        elif self.poly_align_type == 'align_by_roll':
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if 'prim_reg_pred' in pred_results and pred_results['prim_reg_pred'] is not None:
            ...
            loss_poly_cls = self.loss_poly_cls(
                A, B, avg_factor=sum(valid_mask) * self.num_primitive_queries
            )
    ...
    def _forward_head(
        ...
        if return_poly:
            ...
            if self.init_poly_type == 'predicted':
            ...
            elif self.init_poly_type == 'polygonized':
                ...
                poly_pred = self.polygonize_mask(
                    ...
                    cls_pred.detach().cpu(), num_inter=self.num_inter_points,
                ).to(mask_pred.device)

                # poly_pred = self.polygonize_mask_v2(
                #     (mask_pred > 0).detach().cpu().to(torch.uint8).numpy(),
                #     num_inter=self.num_inter_points,
                # ).to(mask_pred.device)
            elif self.init_poly_type == 'none':
                pass
    ...
    def _forward_poly(self, poly_pred, query_feat_list, mask_features):
        ...
        if self.poly_cfg.get('coords', False):
        ...
        if self.poly_cfg.get('decoded_feat', False):
        ...
        prim_pred_reg = prim_pred_base + prim_pred_reg[..., 1:] * self.max_offsets
        # prim_pred_reg = prim_pred_base

        results = dict(
            prim_reg_pred = prim_pred_reg,
            prim_cls_pred = prim_pred_cls,
            prim_base_pred = prim_pred_base,
            prim_ts_pred = t
        )
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        # prim_reg_pred = pred_results['prim_reg_pred'][-1]
