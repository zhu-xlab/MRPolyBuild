...
class PolygonizerHeadV8(MaskFormerHead):
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if 'mask_feat' in pred_results and pred_results['mask_feat'] is not None:
            ...
            prim_ang_targets = torch.zeros(K, N - 1, device=mask_preds.device)
            ...
            if self.poly_cfg.get('pred_angle', False):
                valid_mask = (poly_pred >= 0).all(dim=[1,2])
                prim_ang_targets = prim_ang_targets[valid_mask]

                prim_ang_pred = poly_pred_results['prim_ang_pred'][valid_mask]
                ...
                prim_vec_pred = torch.roll(prim_reg_pred[valid_mask], dims=[1], shifts=[-1]) - prim_reg_pred[valid_mask]
    ...
    def forward(
        ...
        if return_poly:
            ...
            poly_pred_results = self._forward_poly(poly_pred, query_feat_list[-1], mask_features, mask=None)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        prim_cls_pred = torch.zeros(B,Q,N,2)
        ...
        for i in range(B):
            ...
            for j in range(Q):
                ...
                mask = cur_inds & poly_valid_mask

                cur_pred_ring = prim_reg_pred[i,j, mask].view(-1)
