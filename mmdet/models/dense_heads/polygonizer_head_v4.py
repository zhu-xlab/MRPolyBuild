...
class PolygonizerHeadV4(MaskFormerHead):
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json):
        ...
        poly_targets = polygon_utils.align_rings(poly_pred, poly_gt_torch, self.num_inter_points)
    ...
    def _get_poly_targets(self, poly_preds, poly_gt_jsons):
        ...
        if len(poly_preds) == 0:
            return torch.zeros(0, self.num_inter_points, 2), \
                    torch.zeros(0, self.num_primitive_queries, dtype=torch.long), \
                    torch.zeros(0, self.num_primitive_queries, 2)

        poly_reg_targets_list, poly_ind_targets_list, poly_offset_targets_list = [], [], []
        for poly_pred, poly_gt_json in zip(poly_preds.cpu(), poly_gt_jsons):
            gt_aligned, gt_inds, gt_offsets = self._get_poly_targets_single(poly_pred, poly_gt_json)
            poly_reg_targets_list.append(gt_aligned)
            poly_ind_targets_list.append(gt_inds)
            poly_offset_targets_list.append(gt_offsets)

        poly_reg_targets = torch.stack(poly_reg_targets_list, dim=0)
        poly_ind_targets = torch.stack(poly_ind_targets_list, dim=0)
        poly_offset_targets = torch.stack(poly_offset_targets_list, dim=0)

        return poly_reg_targets, poly_ind_targets, poly_offset_targets
    ...
    def _get_targets_single(
        ...
        **pred_results
        ...
        if 'poly_pred' in pred_results:
            ...
            poly_reg_targets, poly_ind_targets, poly_offset_targets = self._get_poly_targets(poly_pred, poly_targets_jsons)
            # min_xy = poly_reg_targets.min(dim=1)[0]
            # max_xy = poly_reg_targets.max(dim=1)[0]
            # center_xy = poly_reg_targets.mean(dim=1)

            # results['poly_reg_targets'] = torch.stack([min_xy, max_xy], dim=1)
            results['poly_reg_targets'] = poly_reg_targets
    ...
    def _forward_head(
        ...
        if return_poly:
            ...
            #     (mask_pred > 0).cpu().to(torch.uint8).numpy(),
    ...
    def _forward_poly(self, poly_pred, query_feat_list, mask_features):
        ...
        # norm_poly_pred = (poly_pred / (H*4.) - 0.5) * 2
        ...
        if self.poly_cfg.get('add_query_feat', False):
