...
class PolygonizerHeadV6(MaskFormerHead):
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json):
        ...
        inter_gt_torch = polygon_utils.interpolate_ring(
            poly_gt_torch, num_bins=self.poly_cfg.get('num_inter_points', 96),
            pad_length=self.poly_cfg.get('num_inter_points', 96)
        )


        if poly_align_type == 'sort_by_angle':
            reg_targets = polygon_utils.sort_rings_by_angle(inter_gt_torch)

        elif poly_align_type == 'align_by_roll':
            reg_targets = polygon_utils.align_rings_by_roll(poly_pred, inter_gt_torch)

        else:
            raise ValueError('No such alignment type!')

        targets['poly_reg_targets'] = reg_targets

        if apply_prim_pred:
            _, pos_inds = (((reg_targets.unsqueeze(0) - poly_gt_torch[:-1].unsqueeze(1)) ** 2).sum(dim=-1) ** 0.5).min(dim=1)
            pos_inds = torch.roll(pos_inds, shifts=[-pos_inds.argmin()])
            # pos_inds = pos_inds.unique()
            pos_inds = torch.cat([pos_inds, pos_inds[:1]])
            if len(pos_inds) > P:
                pos_inds = pos_inds[:P]
                pos_values = reg_targets[pos_inds]
                # pos_values = F.pad(poly_gt_torch[:P], (0, 0, 0, P - len(poly_gt_torch)), value=-1)
            else:
                pos_values = F.pad(reg_targets[pos_inds], (0, 0, 0, P - len(pos_inds)), value=-1)
                pos_inds = F.pad(pos_inds, (0, P - len(pos_inds)), value=-1)

            targets['poly_ind_targets'] = pos_inds
            targets['prim_reg_targets'] = pos_values

        # reg_targets = polygon_utils.sort_rings_by_y(inter_gt_torch)

        # poly_targets = polygon_utils.align_rings_v2(poly_pred, poly_gt_torch, self.num_primitive_queries)
    ...
    def _get_poly_targets(self, poly_preds, poly_gt_jsons):
        ...
        target_dict = {}
        for poly_pred, poly_gt_json in zip(poly_preds.cpu().detach(), poly_gt_jsons):
            cur_target_dict = self._get_poly_targets_single(poly_pred, poly_gt_json)
    ...
    def _get_targets_single(
        ...
        **pred_results
        ...
        if 'poly_pred' in pred_results:
            ...
            target_dict = self._get_poly_targets(poly_pred, poly_targets_jsons)
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if 'prim_reg_pred' in pred_results and pred_results['prim_reg_pred'] is not None:
            ...
            if 'poly_ind_targets' in target_dict:
                ...
                loss_poly_cls = self.loss_poly_cls(
                    A, B, avg_factor=sum(valid_mask) * self.poly_cfg.get('num_primitive_queries', 64)
                )
        ...
        if return_poly:
            ...
            if self.poly_cfg.get('init_poly_type', 'predicted') == 'predicted':
                ...
                poly_pred = self.polygonize_mask(
                    (mask_pred > 0).detach().cpu().to(torch.uint8),
                    cls_pred.detach().cpu(),
    ...
    def _forward_poly(self, poly_pred, query_feat_list, mask_features):
        ...
        if self.poly_cfg.get('use_prim_offsets', False):
            ...
            prim_pred_base = F.grid_sample(
                poly_pred.view(B*Q, 1, N, -1).permute(0,3,1,2),
                torch.cat([t.view(B*Q, 1, P, 1), t.new_zeros(B*Q, 1, P, 1)], dim=-1),
                align_corners=True
            ).permute(0,3,1,2).view(B,Q,P,2)

            prim_pred_reg = prim_pred_base + prim_pred_reg[..., 1:] * self.poly_cfg.get('max_offsets', 10)
            results['prim_base_pred'] = prim_pred_base
            results['prim_ts_pred'] = t

        else:
            prim_pred_reg = (prim_pred_reg[..., :2] + 1) / 2 *  H * 4.
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        if not 'prim_reg_pred' in pred_results:
            # use poly_pred
            ...
            if self.poly_cfg.get('ts_type', 'regression'):
                ...
                for i in range(B):
                    polygons = []
                    for j in range(Q):
                        argmax = prim_inds_pred[i, j].argmax().item()
                        if prim_inds_pred[i,j].max() == self.num_inter_points:
                            cur_pred_ring = prim_reg_pred[i, j, :max(argmax, 3)]
                        else:
                            cur_pred_ring = prim_reg_pred[i,j]
