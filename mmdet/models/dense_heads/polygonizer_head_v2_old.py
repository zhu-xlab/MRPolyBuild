...
class PolygonizerHeadV2(MaskFormerHead):
    ...
    """
    def polygonize_mask(self, imgs, scale=4., num_inter=65):
        B, Q, H, W  = imgs.shape
    ...
    def polygonize_mask(self, imgs, scale=4., num_inter=65):
        ...
        for i in range(B):
            for j in range(Q):
                if imgs[i, j].sum() > 0:
                    pdb.set_trace()
                    rle = mask_util.encode(np.asfortranarray(imgs[i,j]))
                    polygons = mask_util.frPyObjects(rle, H, W)
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json):
        ...
        poly_targets = polygon_utils.align_rings(poly_pred.pred_ori[0], poly_gt_torch, self.num_primitive_queries)
    ...
    def _get_targets_single(self, cls_score: Tensor, mask_pred: Tensor,
        ...
        poly_ind_targets = None
        poly_offset_targets = None
        if poly_pred is not None:
            if type(poly_pred) == torch.Tensor:
                poly_pred = poly_pred[pos_inds]
            elif type(poly_pred) == list:
                poly_pred = [x[pos_inds].detach().cpu() for x in poly_pred]
            elif type(poly_pred) == InstanceData:
                new_poly_pred = InstanceData()
                for key in poly_pred.keys():
                    new_poly_pred[key] = poly_pred[key][pos_inds]
                poly_pred = new_poly_pred
    ...
    def _loss_by_feat_single(self, cls_scores: Tensor, mask_preds: Tensor,
        ...
        if poly_preds is None:
            poly_preds_list = [None] * num_imgs
        elif type(poly_preds) == torch.Tensor or type(poly_preds) == InstanceData:
            poly_preds_list = [x for x in poly_preds]
        elif type(poly_preds) == list:
            # poly_preds_list = [[x[i] for x in poly_preds] for i in range(num_imgs)]
            poly_preds_list = poly_preds
        else:
            raise ValueError()

        (labels_list, label_weights_list, mask_targets_list, mask_weights_list,
         poly_ind_targets_list, poly_offset_targets_list, avg_factor) = \
                self.get_targets(
        ...
        if poly_preds is not None:
            poly_ind_targets = torch.cat(poly_ind_targets_list, dim=0)
            poly_offset_targets = torch.cat(poly_offset_targets_list, dim=0)
            ...
            loss_poly = self.loss_poly(
                ...
                poly_targets[valid_mask],
                avg_factor=len(valid_mask) * poly_preds.shape[1] * 2
            )
    ...
    def forward(self, x: List[Tensor],
        ...
        if self.apply_poly_refine:
            prim_pred = self._forward_poly(poly_pred_list[-1], query_feat_list, mask_features)
            poly_pred_list[-1] = prim_pred
    ...
    def _forward_poly(self, poly_pred, query_feat_list, mask_features):
        ...
        for i in range(B):
            instance = InstanceData(
                pred_reg=prim_pred_reg[i],
                pred_cls=prim_pred_cls[i],
                pred_ori=poly_pred[i]
            )
            prim_pred_list.append(instance)

        return prim_pred_list
