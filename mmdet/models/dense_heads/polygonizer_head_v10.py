...
class PolygonizerHeadV10(MaskFormerHead):
    ...
    def polygonize_mask_v2(self, imgs, scale=4., clockwise=True):
        N, H, W  = imgs.shape
        polygons = []
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json):
        ...
        shift = np.random.randint(0, K - 2)
        inter_poly_pred = torch.roll(inter_poly_pred, shifts=[shift], dims=[0])

        inter_gt_torch = polygon_utils.interpolate_ring(poly_gt_torch, num_bins=K)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        pred_polygons_next = polygon_utils.post_process_without_format(
            prim_reg_pred.cpu(), prim_cls_pred.cpu(), prim_idxes, prim_lens,
        ...
        for idx, polygon in zip(query_idxes, pred_polygons_next):
