...
model = dict(
    ...
    roi_head=dict(
        ...
        poly_head=dict(
            ...
            poly_out_channels=32 * 2,
            mask_size=28,
            num_classes=1,
            loss_poly=dict(
                type='SmoothL1Loss', loss_weight=0.5),
            loss_poly_right_ang=dict(
                type='SmoothL1Loss', loss_weight=1.0),
            poly_cfg=dict(
                lam=2, apply_gcp=False,
                apply_right_angle_loss=True,
                num_base_angles=16
            ),
            poly_decoder=dict(  # Mask2FormerTransformerDecoder
    ...
    train_cfg=dict(
        ...
        rcnn=dict(
            ...
            mask_size=28,
            poly_size=32, # number of sampled points in each polygon
            sample_cfg=dict(num_bins=32),
            pos_weight=-1,
            debug=False)),
    test_cfg=dict(
        rpn=dict(
...
vis_backends = [
    dict(
        ...
        init_kwargs=dict(
            ...
            entity = 'tum-tanmlh',
            name = 'st-mask-rcnn_poly_head_clip_no-gcp_r50_20e_planet_basemap_sample-global-small',
            resume = 'never',
            dir = './work_dirs/',
