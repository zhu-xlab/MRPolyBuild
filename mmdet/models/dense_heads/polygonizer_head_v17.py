...
class PolygonizerHeadV17(MaskFormerHead):
    ...
    def _get_poly_targets_single(self, poly_pred, poly_gt_json, sampled_segments):
        ...
        prim_ref_targets = torch.zeros(P, 2) - 1
        ...
        if self.poly_cfg.get('proj_gt', False):
            if valid_mask.sum() >= 2:
                seg_start, seg_end = seg_inds[valid_mask][[0,-1]]
                # gt_start, gt_end = gt_inds[valid_mask].min() - 1, gt_inds[valid_mask].max() - 1
                matched_gt_segments = poly_gt_torch[gt_inds[valid_mask] - 1]
                matched_pred_segments = poly_pred[seg_start:seg_end+1]
                # matched_gt_segments = poly_gt_torch[gt_start:gt_end+1]

                if matched_gt_segments.std(dim=0).sum() < 1e-8:
                    matched_gt_segments = torch.zeros(0)
                    matched_pred_segments = torch.zeros(0)
                else:
                    matched_gt_segments = polygon_utils.interpolate_segments(
                        matched_gt_segments, num_bins=len(matched_pred_segments)
                    )
                    dis = (((matched_gt_segments - sampled_segments[seg_start:seg_end+1]) ** 2).sum(dim=-1) ** 0.5)
                    if dis.mean() > self.poly_cfg.get('max_match_dis', 10):
                        matched_gt_segments = torch.zeros(0)
                        matched_pred_segments = torch.zeros(0)
            ...
            targets['matched_pred_segments'] = matched_pred_segments
            targets['matched_gt_segments'] = matched_gt_segments
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if 'point_feat' in pred_results and pred_results['point_feat'] is not None:

            B, Q = mask_weights.shape
            P = self.poly_cfg.get('num_primitive_queries', 64)
            N = self.poly_cfg.get('num_inter_points', 96)
            K = mask_weights.sum().int().item()
            point_feat = pred_results['point_feat'];
            ...
            sampled_rings, _, _ = polygon_utils.sample_rings_from_json(
                ...
                num_min_lens=self.poly_cfg.get('num_min_lens', 8)
            )
            sampled_segments = polygon_utils.sample_segments_from_rings(sampled_rings, self.poly_cfg.get('num_inter_points'))

            # print(f'polygonize time: {time.time() - t0}')

            prim_reg_preds = torch.zeros(K, N, 2, device=mask_preds.device)
            prim_ind_targets = torch.zeros(K, P, device=mask_preds.device, dtype=torch.long)
            prim_reg_targets = torch.zeros(K, P, 2, device=mask_preds.device)
            ...
            poly_pred_results = self._forward_poly(
                full_poly_pred, query_feat, point_feat, mask=mask_weights > 0
            )
            ...
            for i in range(K):
                ...
                if self.poly_cfg.get('proj_gt', False):
                    if len(prim_target['matched_pred_segments']) > 0:
                        matched_pred_segments.append(prim_target['matched_pred_segments'])
                        matched_gt_segments.append(prim_target['matched_gt_segments'])

            # print(f'get poly targets time: {time.time() - t0}')
            # dis = [(((x - y) ** 2).sum(dim=-1) ** 0.5).mean() for x, y in zip(matched_pred_segments, matched_gt_segments)]
            ...
            losses['loss_dp'] = opt_dis.mean() * self.poly_cfg.get('loss_weight_dp', 0.01)

            if self.poly_cfg.get('proj_gt', False):

                losses['loss_dp_gt'] = prim_reg_pred[:0].sum() # dummy loss
                if len(matched_gt_segments) > 0:

                    matched_gt_segments = [x.to(prim_reg_pred.device) for x in matched_gt_segments]
                    sizes = torch.tensor([len(x) for x in matched_gt_segments], device=prim_reg_pred.device)
                    pad_gt_segments = polygon_utils.pad_longest_cat(matched_gt_segments)
                    pad_pred_segments = polygon_utils.pad_longest_cat(matched_pred_segments)
                    dp_gt = polygon_utils.batch_decode_ring_dp(
                        pad_pred_segments, sizes, max_step_size=sizes.max(),
                        lam=self.poly_cfg.get('lam', 4),
                        device=prim_reg_pred.device, only_return_dp=True,
                        ref_rings=pad_gt_segments
                    )
                    opt_dis_gt = torch.gather(dp_gt[:,0], 1, sizes.unsqueeze(1)-1)
                    losses['loss_dp_gt'] = opt_dis_gt.mean() * self.poly_cfg.get('loss_weight_dp', 0.01)

                """
                dp_gt = polygon_utils.simplify_rings_dp(
                    matched_pred_segments,
                    max_step_size=self.poly_cfg.get('num_inter_points'),
                    lam=self.poly_cfg.get('lam', 4),
                    device=prim_reg_pred.device,
                    ref_rings=matched_gt_segments,
                    only_return_dp=True
                )
                """

            valid_mask = (poly_pred >= 0).all(dim=-1)
            A = prim_cls_pred[valid_mask].view(-1, self.poly_cfg.get('num_cls_channels', 2))
            B = prim_ind_targets[valid_mask].view(-1)
            loss_poly_cls = self.loss_poly_cls(A, B)
    ...
    def forward(
        ...
        mask_features, point_features, multi_scale_memorys = self.pixel_decoder(x)
        ...
        pred_results = dict(
            ...
            point_feat=[None] * (self.num_transformer_decoder_layers) + [point_features]
        )
        ...
        if return_poly:
            ...
            if len(sampled_segs) > 0:
                query_idx = valid_mask.view(-1).nonzero().view(-1)[segs2poly_idxes[:,0]]
                query_feat = query_feat_list[-1].view(B*Q, -1)[query_idx]
                poly_pred = torch.from_numpy(sampled_segs).to(query_feat.device).float()

                poly_pred_results = self._forward_poly(poly_pred.unsqueeze(0), query_feat.unsqueeze(0), point_features, mask=None)
    ...
    def _forward_poly(self, poly_pred, query_feat, mask_feat, mask=None):
        ...
        if self.poly_cfg.get('point_as_prim', False):
            ...
            query_feat = self.primitive_feat.weight.unsqueeze(0).repeat((K, 1, 1))
            query_embed = self.primitive_embed.weight.unsqueeze(0).repeat((K, 1, 1))
        ...
        if self.poly_cfg.get('point_as_prim', False):
            ...
            prim_pred_reg = torch.clamp(prim_pred_reg, 0, H * 4 - 1)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        if 'seg_idxes' in pred_results:
            ...
            rings, poly2ring_idxes, others = polygon_utils.assemble_segments(
                ...
                cls_pred=prim_cls_pred.cpu()
            )

            sampled_rings, _, _ = polygon_utils.assemble_segments(
