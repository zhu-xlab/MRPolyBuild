...
We provided a model which can perform zero-shot building polygonal mapping with building probability maps given (which is usually generated by a neural networks).

Here we provide an example of converting building probability maps from Google 2.5D Temporal dataset to polygonized buildings. To do this, simply run:
```
...
```
You may would like to configure the paths in `save_cfg` variable in the configuration file.
