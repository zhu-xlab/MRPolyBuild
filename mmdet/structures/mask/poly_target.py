...
def poly_target(pos_proposals_list, pos_assigned_gt_inds_list, gt_masks_list, cfg):
    ...
    poly_targets = map(poly_target_single, pos_proposals_list,
                       pos_assigned_gt_inds_list, gt_masks_list, cfg_list)
    poly_targets = list(poly_targets)
    if len(poly_targets) > 0:
        poly_targets = torch.cat(poly_targets)
    return poly_targets


def poly_target_single(pos_proposals, pos_assigned_gt_inds, gt_masks, cfg):
    ...
    mask_size = (cfg.mask_size, cfg.mask_size)
    ...
    if num_pos > 0:
        ...
        poly_targets = gt_masks.crop_and_resize(
            proposals_np,
            mask_size,
            device=device,
            inds=pos_assigned_gt_inds,
            binarize=binarize,
            clip_boundary=sample_cfg.get('clip_boundary', False)
        )
        temp = [poly_targets.masks[x][0].min() for x in range(len(poly_targets))]
        poly_targets = poly_targets.sample_points(**sample_cfg)
        poly_targets = poly_targets.canonicalize()

        poly_targets = torch.stack([torch.from_numpy(mask[0]).reshape(-1,2) for mask in poly_targets.masks])
        # poly_targets = torch.from_numpy(poly_targets).float().to(device)
    else:
        poly_targets = pos_proposals.new_zeros((0, poly_size, 2))
