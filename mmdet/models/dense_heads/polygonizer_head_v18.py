...
class PolygonizerHeadV18(MaskFormerHead):
    ...
    def _loss_by_feat_single(self, batch_gt_instances: List[InstanceData],
        ...
        if 'mask_feat' in pred_results and pred_results['mask_feat'] is not None:
            ...
            opt_dis = torch.gather(dp, 2, sizes.unsqueeze(1).unsqueeze(1).repeat(1,N,1)-1)
            losses['loss_dp'] = opt_dis.min(dim=1)[0].mean() * self.poly_cfg.get('loss_weight_dp', 0.01)

            dp_points = [x[:-1] for x in dp_points]
            # for x in dp_points: x.requires_grad=True
            ...
            if self.poly_cfg.get('apply_poly_iou_loss', False):
                ...
                # wns = wn_list[0].clamp(0,1)

                # loss_poly_iou = self.loss_dice_wn(wns[None], mask_point_targets[0:1])
                loss_poly_iou = self.loss_dice_wn(wns, mask_point_targets)
                # loss_poly_iou.backward()

                # if dp_points[0].grad.sum().isnan():
                #     pdb.set_trace()
    ...
    def forward(
        ...
        if return_poly:
            ...
            sampled_segs, seg_sizes, poly2segs_idxes, segs2poly_idxes = polygon_utils.sample_segments_from_json(
                ...
                num_min_bins=self.poly_cfg.get('num_min_lens', 8),
                num_bins=self.poly_cfg.get('num_bins', None)
            )
    ...
    def _forward_poly(self, poly_pred, query_feat, mask_feat, mask=None):
        ...
        if self.poly_cfg.get('point_as_prim', False):
            ...
            # if (((poly_pred[0] - prim_pred_reg).abs() < 1e-3) == False).sum() > 0:
            #     pdb.set_trace()
