...
if __name__ == '__main__':
    ...
    # input_image_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/train/images'
    # input_gt_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/train/gt'
    # # save_path = '/home/Datasets/Dataset4EO/AerialImageDataset/train'
    # save_path = '/home/Datasets/Dataset4EO/AerialImageDataset/train_gcp'


    # input_image_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/train/images'
    # input_gt_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/train/gt'
    # save_path = '/home/Datasets/Dataset4EO/AerialImageDataset/val'

    input_image_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/test/images'
    input_gt_path = '/home/Datasets/Dataset4EO/AerialImageDataset/raw/test/gt'
    save_path = '/home/Datasets/Dataset4EO/AerialImageDataset/test'
    ...
    # val_set = [str(i) for i in range(6, 37)]
    val_set = [str(i) for i in range(2, 37)]
        ...
        if label_info[1] not in val_set and label_info[0] in cities:
            ...
            for pid, pa in enumerate(patch_list):
                ...
                for p_im, p_gt in zip(p_ims, p_gts):
                    if np.sum(p_gt > 0) > 5:
                        if poly_type == 'simple':
                            p_polygons = bmask_to_poly(p_gt, True)

                        elif poly_type == 'gcp':
                            """
                            t0 = time.time()
                            gt_poly_jsons = polygon_utils.polygonize_mask(
                                torch.tensor(p_gt), mode='simple_mask', scale=1.
                            )[0]
                            p_polygons = [shapely.geometry.shape(x) for x in gt_poly_jsons]
                            t1 = time.time()
                            t2 = time.time()
                            print(f'time: {t1-t0} {t2-t1}')
                            """

                            p_polygons = bmask_to_poly(p_gt, False)
                            gt_poly_jsons = [mapping(x) for x in p_polygons]

                            # t0 = time.time()
                            p_polygons = polygon_utils.simplify_poly_jsons(
                                gt_poly_jsons, lam=4, max_step_size=128, num_min_bins=16,
                                interval=2, return_format='shapely',
                                device='cuda:0'
                                # device='cpu'
                            )
                            # t1 = time.time()
