...
class PolygonizerHeadV16(MaskFormerHead):
        ...
        if self.poly_cfg.get('proj_gt', False):
            if valid_mask.sum() >= 2:
                ...
                if matched_gt_segments.std(dim=0).sum() < 1e-8:
                    ...
                    if dis.mean() > self.poly_cfg.get('max_match_dis', 1e8):
                        matched_gt_segments = torch.zeros(0)
                        matched_pred_segments = torch.zeros(0)
            else:
                matched_gt_segments = torch.zeros(0)
        ...
        if 'mask_feat' in pred_results and pred_results['mask_feat'] is not None:
            ...
            if self.poly_cfg.get('proj_gt', False):
                ...
                if len(matched_gt_segments) > 0:
                    ...
                    losses['loss_dp_gt'] = opt_dis_gt.mean() * self.poly_cfg.get('loss_weight_dp_gt', 0.01)
    ...
    def predict(self, x: Tuple[Tensor],
        ...
        if 'seg_idxes' in pred_results:
            ...
            if self.poly_cfg.get('use_ref_rings', False):
