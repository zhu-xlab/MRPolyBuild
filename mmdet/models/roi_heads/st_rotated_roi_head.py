...
from mmdet.structures.bbox import bbox2roi, rbbox2roi, obb2hbb, bbox_cxcywh_to_xyxy, obb2xyxy
...
class STRotatedRoIHead(BaseRoIHead):
    """Simplest base roi head including one bbox head and one mask head."""

    def __init__(self, st_cfg={}, poly_head=None, **kwargs):
        self.st_cfg = st_cfg
    ...
    def poly_loss(self, sampling_results, bbox_preds, mask_results, batch_data_samples):
        ...
        if len(valid_idxes) > 0:
            ...
            for i, sampling_result in enumerate(sampling_results):
                ...
                pos_inds.append(
                    torch.ones(
                        sampling_result.pos_priors.shape[0],
                        device=device,
                        dtype=torch.bool))
                pos_inds.append(
                    torch.zeros(
                        sampling_result.neg_priors.shape[0],
                        device=device,
                        dtype=torch.bool))
            ...
            num_classes = 1 if self.bbox_head.reg_class_agnostic else self.bbox_head.num_classes
            roi = pos_rois.repeat_interleave(num_classes, dim=0)
            bbox_preds = bbox_preds.view(-1, self.bbox_head.bbox_coder.encode_size)
            bboxes = self.bbox_head.bbox_coder.decode(
                roi[..., 1:], bbox_preds, max_shape=img_shape)
            """
            bboxes = bbox_preds
            gt_poly_jsons = []
            for i, sampling_result in enumerate(sampling_results):
            ...
            # pred_jsons = polygon_utils.polygonize_mask((mask_preds > 0.5)[:,0].to(torch.int), mode='cv2_single_mask')
            ...
            # pred_shps = [shapely.geometry.shape(x) for x in pred_jsons]
            # gt_shps = [shapely.geometry.shape(x) for x in sampled_gt_jsons]
            # pdb.set_trace()
            # poly_ious = [polygon_utils.cal_iou(x, y) for x, y in zip(pred_shps, gt_shps)]
            ...
            losses_poly = self.poly_head.loss(
                ...
                simple_gt_jsons,
                mask_shape[0],
                mask_feat=sampled_mask_feats, batch_idxes=batch_idxes,
    ...
    def _mask_forward(self,
        ...
        # noisy_mask_feats = self.dropout(mask_feats)
        # mask_preds = self.mask_head(noisy_mask_feats)
        mask_preds = self.mask_head(mask_feats)
        mask_results = dict(mask_preds=mask_preds, mask_feats=mask_feats)

        if self.st_cfg.get('apply_st_mask', False):
            tch_mask_preds = self.tch_mask_head(mask_feats)
            mask_results['tch_mask_preds'] = tch_mask_preds
    ...
    def predict_mask(self,
        ...
        rotated_bboxes = [res.bboxes for res in results_list]
        bboxes = [obb2xyxy(res.bboxes)[:,:4] for res in results_list]
        for i, result in enumerate(results_list):
            result.bboxes = bboxes[i]
        ...
        for i, result in enumerate(results_list):
            result.bboxes = rotated_bboxes[i]
