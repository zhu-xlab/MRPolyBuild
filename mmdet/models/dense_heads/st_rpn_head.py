...
class STRPNHead(AnchorHead):
    ...
    def _init_layers(self) -> None:
        ...
        for param in self.rpn_conv_tch.parameters():
            param.requires_grad = False
        for param in self.rpn_cls_tch.parameters():
            param.requires_grad = False
        for param in self.rpn_reg_tch.parameters():
            param.requires_grad = False
    ...
    def forward_single(self, ori_x: Tensor):
        ...
        if self.st_cfg.get('apply_fft', False):
            ori_x = self.magnitude_mixup(ori_x)
    ...
    def loss_by_feat_single(
        ...
        if not self.st_cfg.get('do_memory_bank', False):
            ...
            ignore_thre = min(ignore_thre, self.st_cfg.get('st_ignore_thr', 0.8))
    ...
    def loss_and_predict(
        ...
        outs_stu = self(x, batch_img_metas)
        outs_tch = self.forward_teacher(x, batch_img_metas)
        outs = outs_stu + outs_tch
        ...
        predictions = self.predict_by_feat(
            *outs_stu, batch_img_metas=batch_img_metas, cfg=proposal_cfg)
    ...
    def forward_teacher(self, x: Tuple[Tensor], batch_img_metas=None) -> Tuple[List[Tensor]]:
        return multi_apply(self.forward_single_teacher, x)

    def forward_single_teacher(self, ori_x: Tensor):

        with torch.no_grad():
            x_tch = self.rpn_conv_tch(ori_x.detach())
            x_tch = F.relu(x_tch, inplace=False)

            rpn_cls_score_tch = self.rpn_cls_tch(x_tch)
            rpn_bbox_pred_tch = self.rpn_reg_tch(x_tch)

        return rpn_cls_score_tch, rpn_bbox_pred_tch
